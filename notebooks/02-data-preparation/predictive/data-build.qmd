...
## Filter to URL-classified relevant events

  ```{r}
  #| label: filter-relevant

  classifications <- read_csv("../../../data/enhanced/url_classifications.csv") |>
    filter(classification == "relevant") |>
    select(globaleventid = GLOBALEVENTID)

  events <- events |>
    semi_join(classifications, by = "globaleventid")

  cat("Relevant events retained:", nrow(events), "\n")
  ```
